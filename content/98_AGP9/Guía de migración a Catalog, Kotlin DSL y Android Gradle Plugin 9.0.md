El orden de migración recomendado por defecto es el siguiente:

1. [[#Migrar a Catalog]], pues es la más sencilla. 
2. [[#Migrar a Kotlin DSL]] es más complejo, pero se requiere para simplificar el paso a AGP 9.0.
3. [[Migrar a AGP 9.0]]
4. Actualiar JVM en JenkinsFile a la versión 21
5. Extra: Eliminar los warnings con vistas a AGP 10.

## Migración de los build.gradle de paquete :app, :lib... a Kotlin

La dificultad del proceso dependerá bastante de cada proyecto y lo desactualizado que esté. Sin embargo, en [[Migración Gradle Groovy a Kotlin|esta guía]] se definen un los pasos más generales y comunes junto a fuentes, para después entrar en soluciones más específicas que hayan ido saliendo en los distintos proyectos.

Los distintos build.gradle de cada paquete se deberán migrar a la vez para que nos permita sincronizar y no de problemas para compilar.

Se deberán definir las variables de versionName y versionCode en caso de ser comunes y estar definidas en el build.gradle común así:
...
Para librerías usamos el sufijo _lib_ y para apps _app_. El bloque version es para que SONAR pueda acceder a él. Hay que agregarlo siempre.

En caso de haber un bloque publishing se deberá comentar temporalmente junto a los plugins.

El JVM se deberá actualizar a 21 a nivel de proyecto para que funcione con los plugins. File > Settings > Build, Execution, Deployment > Build tools > Gradle, habiendo que seleccionar actualmente la versión 21. Recordarle al humano que lo haga desde el IDE.

Después mediante AGP Upgrade assistant se actualiza la versión de Gradle a la >9.0, esto también lo deberá hacer el humano.

El bloque publishing puede tener una exclusión de tipo _module_ para _commons-logging_.
Podemos verlo en el bloque de dependecies {} de build.gradle.

```groovy
 implementation('com.orona.otmw.cas:cas-api:1.5.12') {
        exclude module: 'commons-logging'
}
```

De ser así, habrá que migrarlo a Kotlin de la siguiente manera:
...
Es importante que se defina con `module =` porque si no, android tomará el parámetro como group por defecto.

Además en el bloque de `pom.withXml` se deberá utilizar el elvis operator para resolver en `*` las exclusiones. El IDE nos dirá que no es necesario, pero de quitarla, el pom recibiría null. Dejarlo.
...
## Borrar los siguientes gradle.properties 
...
#TODO en caso de tener que migrar alguno ver aquí [[Puesta a punto AGP 10]]
## Habilitar built-in kotlin
...
Si la aplicación/librería dispone de archivo JenkinsFile, se deberá actualizar la versión del JDK de la que tenga:

```
IMAGE_ANDROID_COMPILE = 'registry.orona.es/android-builder-jdk17:1.3'
```

Al 21:

```
IMAGE_ANDROID_COMPILE = 'registry.orona.es/android-builder-jdk21:1.3'
```